    ...
    <Box sx={{ maxWidth: 1120, mx: "auto", pb: 5 }}>
      ...
      <Paper elevation={0} sx={{ p: { xs: 2.25, md: 3 }, borderRadius: 3, bgcolor: "rgba(17,24,39,0.9)", border: "1px solid rgba(96,165,250,0.18)", boxShadow: "0 18px 45px rgba(0,0,0,0.16)" }}>
        ...
        <Stack spacing={2.5}>
          ...
          {role === "admin" && <Paper elevation={0} sx={{ p: { xs: 2, md: 2.2 }, mt: 0.5, bgcolor: "rgba(79,140,255,0.055)", border: "1px solid rgba(96,165,250,0.16)", borderRadius: 2.5 }}><Typography variant="subtitle1" fontWeight={800}>Administration</Typography><Typography variant="body2" color="text.secondary" sx={{ mt: 0.8, mb: 2.1 }}>Manage developer assignment and workflow status. Priority is set by the requester and is read-only.</Typography><Stack direction={{ xs: "column", md: "row" }} spacing={2}><FormControl fullWidth size="small"><InputLabel id="developer-label">Assigned developer</InputLabel><Select labelId="developer-label" label="Assigned developer" value={draft.assignedDeveloperId ?? ""} onChange={(event) => { const id = event.target.value; const dev = developers.find((item) => item.id === id); setDraft((current) => ({ ...current, assignedDeveloperId: id || undefined, assignedDeveloperName: dev?.name })); }}>{<MenuItem value="">Unassigned</MenuItem>}{developers.map((dev) => <MenuItem key={dev.id} value={dev.id}>{dev.name}</MenuItem>)}</Select></FormControl><FormControl fullWidth size="small"><InputLabel id="status-label">Status</InputLabel><Select labelId="status-label" label="Status" value={draft.status} onChange={(event) => setDraft((current) => ({ ...current, status: event.target.value as TicketStatus }))}>{workflow.map((status) => <MenuItem key={status} value={status}>{status}</MenuItem>)}</Select></FormControl><Paper elevation={0} sx={{ flex: 1, minWidth: 180, px: 1.5, py: 1.15, bgcolor: "rgba(15,23,42,0.56)", border: "1px solid rgba(148,163,184,0.10)", borderRadius: 2 }}><Typography variant="caption" color="text.secondary">Priority (requester set)</Typography><Box sx={{ mt: 0.45 }}><PriorityBadge priority={draft.priority} /></Box></Paper></Stack><Stack direction="row" alignItems="center" justifyContent="flex-end" spacing={1.2} sx={{ mt: 2 }}>{saved && <Typography variant="caption" color="success.main">Saved</Typography>}<Button variant="contained" startIcon={<SaveOutlinedIcon />} onClick={handleSave}>Save Changes</Button></Stack></Paper>}

          {role === "developer" && <Paper elevation={0} sx={{ p: { xs: 2, md: 2.2 }, mt: 0.5, bgcolor: "rgba(79,140,255,0.055)", border: "1px solid rgba(96,165,250,0.16)", borderRadius: 2.5 }}><Typography variant="subtitle1" fontWeight={800}>Developer Actions</Typography><Typography variant="body2" color="text.secondary" sx={{ mt: 0.8, mb: 2.1 }}>Update the workflow status for work assigned to you. Priority remains controlled by the requester.</Typography><Stack direction={{ xs: "column", sm: "row" }} spacing={2} alignItems={{ xs: "stretch", sm: "center" }}><FormControl size="small" fullWidth sx={{ maxWidth: 360 }}><InputLabel id="developer-status-label">Status</InputLabel><Select labelId="developer-status-label" label="Status" value={draft.status} onChange={(event) => setDraft((current) => ({ ...current, status: event.target.value as TicketStatus }))}>{workflow.map((status) => <MenuItem key={status} value={status}>{status}</MenuItem>)}</Select></FormControl><Stack direction="row" spacing={1.2} alignItems="center" justifyContent="flex-end" sx={{ flex: 1 }}>{saved && <Typography variant="caption" color="success.main">Saved</Typography>}<Button variant="contained" startIcon={<SaveOutlinedIcon />} onClick={handleSave}>Save Changes</Button></Stack></Stack></Paper>}
          ...
          {role === "viewer" && <Box sx={{ p: 1.7, borderRadius: 2.5, bgcolor: "rgba(148,163,184,0.05)", border: "1px solid rgba(148,163,184,0.10)" }}><Typography fontWeight={800}>Read-only access</Typography><Typography variant="body2" color="text.secondary" mt={0.35}>This account can view all request and internal assignment information but cannot make changes.</Typography></Box>}
