...
import { Box, Chip, InputAdornment, Paper, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, TextField, Typography } from "@mui/material";
...
export default function RecentTable({ tickets, title = "Recent Requests", subtitle = "Latest change requests and reporting/data issues across PTP, RTR, and DFS.", onTicketClick }: RecentTableProps) {
  ...
  const filteredTickets = useMemo(() => {
    ...
    return tickets.filter((ticket) => [ticket.id, ticket.title, ticket.requestType, ticket.tower, ticket.requester].some((value) => value.toLowerCase().includes(normalized))).slice(0, 10);
  }, [query, tickets]);
    ...
    <Paper elevation={0} sx={{ mt: 2.5, overflow: "hidden", border: "1px solid rgba(148,163,184,0.12)", borderRadius: 3, bgcolor: "rgba(17,24,39,0.82)", boxShadow: "0 14px 34px rgba(2,6,23,0.2)" }}>
      ...
      <TableContainer sx={{ maxHeight: 430 }}>
        <Table stickyHeader size="small" sx={{ minWidth: 900 }}>
          <TableHead><TableRow>{["CR Number", "Type", "Title", "Tower", "Priority", "Status", "Requester", "ETA"].map((label) => <TableCell key={label} sx={{ fontWeight: 800, bgcolor: "#162033", borderBottomColor: "rgba(148,163,184,0.14)", whiteSpace: "nowrap" }}>{label}</TableCell>)}</TableRow></TableHead>
          <TableBody>
