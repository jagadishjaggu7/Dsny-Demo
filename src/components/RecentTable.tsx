...
type SortKey = "id" | "requestType" | "title" | "tower" | "deliveryCentre" | "priority" | "status" | "requester" | "eta";
...
const sortableHeaders: Array<{ key: SortKey; label: string }> = [
  ...
  { key: "deliveryCentre", label: "Delivery Centre" },
...
export default function RecentTable({ tickets, title = "Recent Requests", subtitle = "Latest change requests and reporting/data issues across PTP, RTR, and DFS.", onTicketClick }: RecentTableProps) {
  ...
  const filteredTickets = useMemo(() => {
    ...
    const searched = normalized
      ? tickets.filter((ticket) => [ticket.id, ticket.title, ticket.requestType, ticket.tower, ticket.deliveryCentre ?? "", ticket.requester].some((value) => value.toLowerCase().includes(normalized)))
      : [...tickets];
    ...
    const getSortValue = (ticket: Ticket): string | number => {
      switch (sortKey) {
        ...
        case "deliveryCentre": return ticket.deliveryCentre ?? "";
  ...
  return (
    <Paper elevation={0} sx={{ mt: 2.5, overflow: "hidden", border: "1px solid rgba(148,163,184,0.12)", borderRadius: 3, bgcolor: "rgba(17,24,39,0.82)", boxShadow: "0 14px 34px rgba(2,6,23,0.2)" }}>
      <Box sx={{ px: { xs: 2, sm: 2.5 }, py: 2, display: "flex", gap: 2, alignItems: { xs: "stretch", sm: "center" }, justifyContent: "space-between", flexDirection: { xs: "column", sm: "row" }, borderBottom: "1px solid rgba(148,163,184,0.10)" }}>
        ...
        <TextField size="small" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search ID, title, type, tower, delivery centre or requester" aria-label="Search requests" sx={{ width: { xs: "100%", sm: 390 }, "& .MuiOutlinedInput-root": { borderRadius: 2, bgcolor: "rgba(15,23,42,0.7)" } }} InputProps={{ startAdornment: <InputAdornment position="start"><SearchIcon fontSize="small" /></InputAdornment> }} />
      </Box>
        ...
        <Table stickyHeader size="small" sx={{ minWidth: 1080 }}>
          ...
          <TableBody>
            {filteredTickets.map((ticket) => <TableRow key={ticket.id} hover onClick={() => onTicketClick?.(ticket)} sx={{ cursor: onTicketClick ? "pointer" : "default" }}>
              ...
              <TableCell><Chip label={ticket.tower} size="small" variant="outlined" /></TableCell>
              <TableCell sx={{ minWidth: 170 }}>{ticket.deliveryCentre ? <Chip label={ticket.deliveryCentre} size="small" variant="outlined" /> : <Typography variant="caption" color="text.secondary">Not set</Typography>}</TableCell>
            ...
            {filteredTickets.length === 0 && <TableRow><TableCell colSpan={9} align="center" sx={{ py: 5 }}><Typography color="text.secondary">No requests match your search.</Typography></TableCell></TableRow>}
          </TableBody>
        </Table>
      </TableContainer>
    </Paper>
