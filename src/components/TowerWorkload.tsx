    ...
    <Box sx={{ minWidth: 0 }}>
      <Stack direction="row" justifyContent="space-between" alignItems="center" mb={1.75}>
        <Box minWidth={0}>
          ...
          <Typography variant="body2" color="text.secondary" mt={0.3}>Request volume across business towers</Typography>
        </Box>
      ...
      <Stack spacing={1.55}>
        {entries.map(([tower, count]) => {
          ...
          return (
            <Box key={tower}>
              <Box sx={{ display: "grid", gridTemplateColumns: "minmax(0,1fr) 72px", columnGap: 1.5, alignItems: "center", mb: 0.6 }}>
                <Stack direction="row" spacing={0.9} alignItems="center" minWidth={0}>
                  ...
                  <Box minWidth={0}>
                    <Typography variant="body2" fontWeight={800} lineHeight={1.15} noWrap>{tower}</Typography>
                    <Typography variant="caption" color="text.secondary" lineHeight={1.15} noWrap>{meta.label}</Typography>
                  </Box>
                </Stack>
